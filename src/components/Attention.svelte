				...
				>
					<div class="qkv flex h-full flex-col justify-center gap-[5rem] pl-[6rem]">
						<div class="column key">
							<div class="head1 title">Kunci</div>
							...
							{#each $tokens as token, index}
								...
								<Tooltip placement="right" class="popover"
									>Kunci, Kepala {$attentionHeadIdx + 1}, vektor({$modelMeta.dimension /
										$modelMeta.attention_head_num})</Tooltip
								>
							{/each}
						</div>
						<div class="column query">
							<div class="head1 title">Pertanyaan</div>
							{#each $tokens as token, index}
								<div
								...
								<Tooltip placement="right" class="popover"
									>Pertanyaan, Kepala {$attentionHeadIdx + 1}, vektor({$modelMeta.dimension /
										$modelMeta.attention_head_num})</Tooltip
								>
							{/each}
						</div>
						<div class="column value">
							<div class="head1 title">Nilai</div>
							{#each $tokens as token, index}
								...
								<Tooltip placement="right" class="popover"
									>Nilai, Kepala {$attentionHeadIdx + 1}, vektor({$modelMeta.dimension /
										$modelMeta.attention_head_num})</Tooltip
								>
							{/each}
						</div>
					...
					<div class="head-out mx-[2rem]">
						<div class="column out">
							<div class="head1 title">Keluaran</div>
							{#each $tokens as token, index}
