	...
	const animateForwardTransition = () => {
		...
		tl = gsap.timeline({
			...
			onComplete: () => {
				...
				const shiftedHead = heads.shift();
				if (shiftedHead) {
					heads.push(shiftedHead);
				}
	...
	const animateBackwardTransition = () => {
		...
		tl = gsap.timeline({
			...
			onComplete: () => {
				...
				const poppedHead = heads.pop();
				if (poppedHead) {
					heads.unshift(poppedHead);
				}
				disablePagination = false;
			}
		});
...
<div class="multi-head flex w-full" data-click="attention-head">
	<div class="head-title absolute bottom-2 right-3 text-right text-gray-400">
		<span class="title-text"
			>Kepala {$attentionHeadIdxTemp + 1} daripada {$modelMeta.attention_head_num}</span
		>
		<button
