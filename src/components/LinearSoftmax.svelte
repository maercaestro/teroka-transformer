<script lang="ts">
	...
	const onClickSoftmaxTitle = (e: Event) => {
		e.stopPropagation();
		e.preventDefault();
		if (!isSoftmaxExpanded) {
			expandedBlock.set({ id: blockId });
	...
	function handleOutsideClick(e: Event) {
		if (isSoftmaxExpanded && expandableEl && !expandableEl.contains(e.target as Node)) {
			expandedBlock.set({ id: null });
		}
	}
	onMount(() => {
...
>
	...
	>
		<div class="flex items-center gap-1">Kebarangkalian <ZoomInOutline></ZoomInOutline></div>
	</div>
	<div
				...
				<div class="softmax-subtitle softmax-detail flex text-center text-xs opacity-0">
					...
					<div class="title-box probability">
						<div class="title-text mr-1">
							{$sampling.type === 'top-k' ? 'Softmax' : 'Penormalan'}
						</div>
					</div>
				</div>
			{/if}
