...
<Card class={'softmax-popover popover bg-white text-sm'}>
	...
	>
		<h3 class="font-semibold text-gray-900">
			Kebarangkalian token <span class="highlight">"{selected?.token}"</span> akan dipilih
		</h3>
	</div>
	<div class="softmax-popover-content">
		<div class="formula-steps">
			<div class="formula-step">
				<div class="step-title">Logit diskala</div>
				<div class="step-content">
					<div class="fraction">
						...
						<div class="frac-bottom relative">
							...
							<span class="annotation temp"> <Arrow size={16} /><span>suhu</span></span>
						</div>
					</div>
				</div>
			</div>
						...
						{:else}
							<div class="cases">
								<div class="case-row">
									...
									<span class="number" class:highlight={selected?.rank != null && selected?.cutoffIndex != null && selected.rank <= selected.cutoffIndex}
								...
								<div class="case-row">
									<span class="condition text"
										>Otherwise <span
											class="number infinity"
											class:highlight={selected?.rank != null && selected?.cutoffIndex != null && selected.rank > selected.cutoffIndex}>0</span
										></span
									>
								</div>
							</div>
							...
							<div class="frac-top">
								...
								>
									{#if selected?.topKLogit === -Infinity}
										<Katex math={'-\\infty'} />
									{:else}
										{getStringNumber(selected?.topKLogit ?? 0)}
									{/if}
								</span>)
							</div>
							<div class="frac-line"></div>
									...
									<span
										...
										>
											{#if item?.topKLogit === -Infinity}
												<Katex math={'-\\infty'} />
											{:else}
												{getStringNumber(item?.topKLogit ?? 0)}
											{/if}
										</span>) +{' '}
									</span>
								{/each}
			...
			{:else}
				<div class="formula-step norm-step">
					<div class="step-title">Penormalan</div>
					<div class="step-content">
						{#if selected?.cutoffIndex != null && selected?.rank != null && selected.cutoffIndex >= selected.rank}
							<div class="fraction">
								<div class="frac-top">
									<span
										class="highlight number"
										class:filtered={selected?.cutoffIndex != null && selected?.rank != null && selected.cutoffIndex >= selected.rank}
										>{getStringNumber(selected?.topPProbability)}</span
									>
								</div>
								<div class="frac-line"></div>
								<div class="frac-bottom">
									{#each data.slice(0, 4) as item, idx}
										{#if item.cutoffIndex != null && idx <= item.cutoffIndex}
											<div>
												<span
													class:highlight={idx === selected?.rank}
													class="number"
													class:filtered={true}
													>{getStringNumber(
														item.cutoffIndex != null && idx > item.cutoffIndex ? 0 : item.topPProbability
													)}</span
												>
										...
										{/if}
									{/each}
									{#if selected?.cutoffIndex != null && 4 <= selected.cutoffIndex}
