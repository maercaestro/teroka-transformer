<script lang="ts">
	...
	const matrixColorScale =
		typeof colorScale === 'function'
			? colorScale
			: d3.interpolate('white', (theme.colors as any)[colorKey]?.[400] || '#3b82f6');
	...
	function onCellOver(this: any, e: any, d: any) {
		const el = this;
		onMouseOverCell?.(e, d, el);

		const tooltipResult = showTooltip?.(e, d.cell, el);
		if (tooltipResult) visibleTooltip(e, tooltipResult);
	}

	function onCellOut(this: any, e: any, d: any) {
		const el = this;
		onMouseOutCell?.(e, d, el);
		hideTooltip();
	}

	function onSvgOut(e: any, d: any) {
		onMouseOutSvg?.(e, d);
		hideTooltip();
	}

	function visibleTooltip(event: Event, data: any) {
		tooltipData = data;
		tooltipVisible = true;
