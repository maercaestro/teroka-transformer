...
export const runModel = async ({
	...
	// Convert logits to number array if needed
	const numericLogits = Array.isArray(logits) 
		? logits.map(l => typeof l === 'string' ? parseFloat(l) : Number(l)) 
		: Array.from(logits as ArrayLike<number>);
		
	const { probabilities, sampled } = getProbabilities({ tokenizer, logits: numericLogits, sampling, temperature });

	modelData.set({ logits: numericLogits, outputs, probabilities, sampled });
...
const setPredictedTokenForAnimation = (probabilities: Probabilities, sampled: Probability, sampling: any) => {
	let delay = 10;
	...
	try {
		...
		const outputs = targetTensors.reduce(
			(obj, key) => {
				...
				const processedData = {
					// ...out,
					data: reshapeArray([...out.data] as number[], out.dims as number[])
				};
